  ...
  loading: () => (
    <div className="absolute inset-0 flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="w-12 h-12 border-4 border-white/5 border-t-brand rounded-full animate-spin" />
...
function WorkflowCard({ workflow, onClick, activeTab, onRename, onDelete }) {
  ...
  return (
    <div
      ...
      className="group relative aspect-[3/4] rounded-lg overflow-hidden cursor-pointer border border-white/5 bg-[#0a1422] transition-all hover:border-brand/30 hover:scale-[1.02] shadow-2xl"
    >
      {workflow.thumbnail ? (
        <img
          ...
          {showOptions && (
            <div className="absolute top-10 right-0 w-32 bg-[#111] border border-white/10 rounded-lg shadow-2xl py-1 animate-in fade-in zoom-in duration-200">
              <button
                ...
                className="w-full px-4 py-2 text-left text-[11px] font-bold text-white/70 hover:text-brand hover:bg-white/5 transition-colors flex items-center gap-2"
              >
...
export default function WorkflowStudio({
  ...
  // Catch-all studio routes (/studio/[[...slug]], /zh/studio/[[...slug]]) expose segments as
  // params.slug; params.tab is an array only on a [[...tab]] catch-all. Workflow links navigate to
  // /workflow/[id]/[tab] (and /studio/workflows/* is redirected below), so this fallback is best-effort.
  ...
  useEffect(() => {
    ...
    loadWorkflows();
  }, [apiKey, activeMainTab, reloadTick]);
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${
                      activeSubTab === "playground"
                        ? "bg-brand text-on-brand shadow-[0_0_15px_rgba(46,230,214,0.2)]"
            ...
            <div className="flex bg-white/5 p-1 rounded-lg">
               <button
                 ...
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "playground" ? "bg-brand text-on-brand" : "text-white/40"
                 }`}
               >
                 Play
               </button>
               <button
                 ...
                 type="button"
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "builder" ? "bg-brand text-on-brand" : "text-white/40"
                 }`}
               >
                 Builder
               </button>
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
          {activeSubTab === "playground" ? (
            <>
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  <form onSubmit={handleRun} className="space-y-6">
                              ...
                              <div key={key} className="space-y-2">
                                ...
                                {prop.type === "string" && !prop.enum ? (
                                  <textarea
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-brand/50 transition-colors min-h-[80px] resize-none"
                                    placeholder={
                                  ...
                                  <select
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-brand/50 transition-colors"
                                  >
                                    {prop.enum.map((opt) => (
                                      <option
                                        key={opt}
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-brand text-on-brand text-xs font-black uppercase tracking-[0.2em] rounded-xl hover:bg-brand-hover transition-all transform hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:grayscale shadow-[0_0_30px_rgba(46,230,214,0.15)] flex items-center justify-center gap-3 mt-8"
                    >
                      {isExecuting ? (
                        <>
              ...
              {/* Preview Panel */}
              <div className="flex-1 overflow-y-auto p-8 lg:p-12 bg-[#050b14] flex items-center justify-center min-h-[500px]">
                {error && (
                ...
                {isExecuting && (
                  <div className="flex flex-col items-center gap-6 animate-fade-in">
                    ...
                    <div className="text-center space-y-2">
                      <div className="text-[10px] font-black text-brand uppercase tracking-[0.3em] animate-pulse">
                        Running Pipeline
                      </div>
                  ...
                  <div className="w-full max-w-4xl space-y-8 animate-fade-in-up">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {result.outputs?.map((out, idx) => (
                        <div
                          key={idx}
                          className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-brand/30 transition-all shadow-2xl"
                        >
                          {out.type === "image_url" ? (
                            <img
                              src={out.value}
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform">
                            <div className="flex items-center justify-between">
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-brand hover:text-on-brand transition-colors"
                              >
                                <svg
                                  width="14"
                                  height="14"
      ...
      <div className="max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="text-white/40 text-sm font-medium">
                Chain steps together — script → voice → video.
              </p>
            </div>
            <button
              onClick={() => handleCreateWorkflow()}
              className="px-6 py-3 bg-brand text-on-brand text-xs font-black uppercase tracking-widest rounded-lg hover:bg-brand-hover transition-all transform hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(46,230,214,0.3)] flex items-center gap-2"
            >
              <svg
                width="14"
                height="14"
        ...
        {loading ? (
          ...
          <div className="py-24 text-center border border-pop/30 rounded-2xl bg-pop/5" role="alert">
            <p className="text-sm text-white/70">Couldn&apos;t load workflows.</p>
            <button
              type="button"
      ...
      {renamingWorkflow && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <form 
            ...
            className="relative w-full max-w-sm bg-[#0a1422] border border-white/10 rounded-2xl p-8 shadow-2xl animate-in fade-in zoom-in duration-300"
          >
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-brand uppercase tracking-widest">Workflow Name</label>
                <input
                  autoFocus
                  type="text"
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-brand text-on-brand px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-brand-hover transition-all transform hover:scale-105 active:scale-95"
                >
                  Save Name
                </button>
              </div>
