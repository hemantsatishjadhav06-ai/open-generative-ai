...
const SELECT_CLASS = "w-full rounded-lg border border-white/10 bg-surface-panel px-3 py-2 text-xs text-white outline-none focus:border-brand/50";
...
export function VideoSettingsControl({
  ...
  const renderProfile = (option) => {
    ...
    return (
      <label
        key={option.value}
        className={`flex items-start gap-3 rounded-lg px-3 py-2 text-xs transition-colors focus-within:ring-1 focus-within:ring-brand/50 ${
    ...
    <fieldset className="min-w-0">
      ...
      {onDefaultResolution && (
        <button type="button" onClick={onDefaultResolution}
          className="mt-2 px-3 py-2 text-xs text-brand hover:underline focus-visible:outline focus-visible:outline-1 focus-visible:outline-brand">
