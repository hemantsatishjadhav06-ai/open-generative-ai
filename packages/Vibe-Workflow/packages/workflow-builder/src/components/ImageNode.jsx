...
import { api, errorMessage, isFetchableMedia } from "./gatewayClient";
import { t } from "./i18n";
...
const ImageGeneration = ({ id, data, selected }) => {
  ...
  useEffect(() => {
    if (currentOutput) {
      ...
      (isFetchableMedia(currentOutput) ? fetch(currentOutput, { method: 'HEAD' }) : Promise.reject(new Error('skip')))
        .then(res => {
  ...
  const updateWorkflowThumbnail = async (thumbnail) => {
    ...
    try { 
      const response = await api.post(`/api/workflow/${workflow_id}/thumbnail`, { 
        thumbnail 
      });
      if (response.data.success) toast.success("Cover image updated successfully");
    } catch(error) {
      toast.error(errorMessage(error, t("thumbnailFailed")));
    };
  };
