    ...
    const showDropdown = (type, anchorBtn) => {
        ...
        if (type === 'model') {
            ...
            const makeModelItem = (m, isV2V = false) => {
                ...
                const iconColor = isV2V ? 'bg-orange-500/10 text-orange-400' : m.id.includes('kling') ? 'bg-blue-500/10 text-blue-400' : m.id.includes('veo') ? 'bg-pop-500/10 text-pop-400' : m.id.includes('sora') ? 'bg-rose-500/10 text-rose-400' : 'bg-primary/10 text-primary';
                item.innerHTML = `
