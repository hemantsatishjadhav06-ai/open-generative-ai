...
export function CinemaStudio() {
    ...
    overlayContent.className = 'w-full max-w-4xl bg-[#0f1c2e] border border-white/10 rounded-3xl p-4 md:p-8 shadow-2xl transform scale-95 transition-transform duration-300 flex flex-col max-h-[90vh]';
    ...
    promptBar.className = 'bg-[#15253b] border border-white/10 rounded-[2rem] p-4 flex justify-between shadow-3xl items-end relative';
    ...
    const createDropdown = (items, selected, onSelect, trigger) => {
        ...
        menu.className = 'custom-dropdown fixed bg-[#15253b] border border-white/10 rounded-xl py-1 shadow-2xl z-50 flex flex-col min-w-[100px] animate-fade-in';
    ...
    summaryCard.className = 'flex flex-col items-start justify-center px-4 py-2 bg-[#1a2d47] rounded-xl border border-white/5 hover:border-white/20 transition-colors text-left flex-1 min-w-[100px] md:min-w-[140px] max-w-[240px] h-[56px] relative group overflow-hidden';
    ...
    generateBtn.className = 'h-[56px] px-8 bg-[#2ee6d6] text-black rounded-xl font-black text-xs uppercase hover:bg-white transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed';
    ...
    builderCard.className = 'bg-[#15253b] border border-white/10 rounded-2xl p-4 shadow-3xl';
    ...
    builderCard.innerHTML = `
        ...
        <div class="flex flex-col gap-2">
            ...
            <button id="apply-builder-btn" class="px-4 py-2 bg-primary text-on-brand rounded-lg text-xs font-bold hover:shadow-glow transition-all">
                ${t('cinema.useSetup')}
            </button>
        </div>
    `;
    ...
    const createActionBtn = (label, primary = false) => {
        ...
        btn.className = primary
            ? 'bg-[#2ee6d6] text-black px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wide hover:bg-white transition-colors shadow-glow-sm hover:scale-105 active:scale-95'
    ...
    const renderHistory = () => {
        ...
        generationHistory.forEach((entry, idx) => {
            ...
            thumb.className = `relative group/thumb cursor-pointer rounded-lg overflow-hidden border-2 transition-all duration-300 aspect-square ${idx === 0 ? 'border-[#2ee6d6] shadow-glow-sm' : 'border-white/10 hover:border-white/30'}`;
    ...
    const loadHistoryItem = (entry, thumbElement) => {
        ...
        if (thumbElement) {
            historyList.querySelectorAll('div').forEach(t => {
                t.classList.remove('border-[#2ee6d6]', 'shadow-glow-sm');
                t.classList.add('border-white/10');
            });
            thumbElement.classList.remove('border-white/10');
            thumbElement.classList.add('border-[#2ee6d6]', 'shadow-glow-sm');
        }
    };
