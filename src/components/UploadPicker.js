...
export function createUploadPicker({ anchorContainer, onSelect, onClear, maxImages: initialMaxImages = 1, uploadFn, requiresSession }) {
    ...
    const renderPanel = () => {
        ...
        if (isMulti && selectedEntries.length > 0) {
            ...
            doneBtn.type = 'button';
            doneBtn.className = 'flex items-center gap-1 px-3 py-1.5 bg-primary text-on-brand rounded-xl text-xs font-black transition-all hover:scale-105';
        ...
        if (isMulti && selectedEntries.length > 0) {
            ...
            doneBtn2.className = 'px-4 py-1.5 bg-primary text-on-brand rounded-xl text-xs font-black transition-all hover:scale-105';
