...
function BinaryStatusBar(onStatusChange) {
    ...
    const refresh = async () => {
        ...
        if (status.exists) {
            ...
            btn.textContent = t('localModels.installEngine');
            btn.className = 'px-3 py-1.5 rounded-lg text-xs font-bold bg-primary text-on-brand transition-all';
...
function Wan2gpConfigBar(onChange) {
    ...
    wrap.innerHTML = `
        ...
        <div class="flex items-center gap-2">
            ...
            <button id="wan2gp-save" class="px-3 py-1.5 rounded-lg text-xs font-bold bg-primary text-on-brand hover:shadow-glow transition-all">Save</button>
        </div>
    ...
    card.innerHTML = `
        <div class="flex flex-col gap-1 min-w-0">
            ...
            <div class="flex items-center gap-1.5 flex-wrap mt-1">
                <span class="px-1.5 py-0.5 rounded-md text-[10px] font-bold ${model.type === 'video' ? 'bg-pop-500/15 text-pop-300' : 'bg-primary/10 text-primary'}">${model.type.toUpperCase()}</span>
        ...
        <div class="flex items-start justify-between gap-3">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ${isDownloaded
                    ...
                    : `<button class="download-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-primary text-on-brand hover:shadow-glow transition-all">${DownloadIcon} ${t('localModels.download')}</button>`
                }
            </div>
        </div>
