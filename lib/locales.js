// Locale registry for the standalone Aquora studio app.
//
// Additive-route architecture: English stays the
...
// Derive the active locale purely from the URL path, no
// cookies/headers/negotiation.
...
// Prefixes any app path with the locale root, e.g.
// localizePath('zh', '/agents/abc') -> '/zh/agents/abc'. Used by the route
// trees mirrored per locale (/agents, /workflow).
export function localizePath(locale, path) {
  const config = getLocaleConfig(locale);
  return `${config.rootPath}${path.startsWith('/') ? path : `/${path}`}`;
}

// Page <title>s of the full-screen /agents and /workflow pages.
export function pageTitle(locale, key, values) {
  const pages = getCommonCopy(locale).meta?.pages || {};
  return fillCopy(pages[key] || getCommonCopy(DEFAULT_LOCALE).meta.pages[key] || 'Aquora', values);
}

// Fills {name} placeholders in a copy string, e.g.
// fillCopy('Today: {spent} of {cap}', { spent: '$1', cap: '$10' }).
