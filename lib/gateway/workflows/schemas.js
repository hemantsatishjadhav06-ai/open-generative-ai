...
export async function nodeSchemas({ locale = 'en' } = {}) {
    ...
    for (const entry of entries) {
        ...
        // Extend / multi-step models need a prior studio clip: studio only.
        if (entry.resolve || entry.steps) continue;
...
export async function resolveNodeModel(category, model) {
    ...
    if (!entry || !entry.enabled || !entry.fal || entry.resolve || entry.steps) return null;
