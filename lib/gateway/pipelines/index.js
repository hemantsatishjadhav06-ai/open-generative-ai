...
const FILES = { 'ai-clipping': 'clipping' };
...
export async function startPipelineJob({ name, input = {}, session }) {
    const def = await loadPipeline(name);
    if (!def) throw new GatewayError(404, 'unknown_model', "This feature isn't available.");
    let value = input;
