...
import { decodeJobToken, loadJob, newJobId, signJobToken } from './jobs.js';
...
// A token names a prior generation of the SAME session (a fal job, or a
// catalog-steps pipeline job such as an earlier extend); its output video is
// the source clip. Tokens of other sessions, expired or tampered tokens and
// unfinished jobs resolve to null.
export async function sourceUrlFromToken(token, session) {
    const claims = decodeJobToken(token);
    if (!claims || !session?.sid || claims.sid !== session.sid) return null;
    if (claims.k === 'pipeline') {
        const job = await loadJob(claims.r, claims.w || session.cid);
        if (!job || job.sid !== session.sid || job.status !== 'completed') return null;
        const result = job.result || {};
        const url = result.video?.url || result.url || (Array.isArray(result.outputs) ? result.outputs[0] : null);
        return typeof url === 'string' && /^https?:\/\//i.test(url) ? url : null;
    }
    if (claims.k !== 'fal') return null;
    const cached = recall(claims.r);
    if (cached) return cached.status === 'completed' ? cached.video?.url || cached.url || null : null;
    try {
...
// Entries with `resolve` (extend / 4K re-render) need the source clip: a
// missing or unusable token is a 400 the studio can show. Other entries keep
// the lenient legacy behaviour (a stray request_id is dropped, or fills
// video_url when it resolves). → {payload, sourceUrl}
export async function resolveReferencedJob(catalog, entry, payload, session) {
    const field = typeof catalog?.sourceField === 'function' ? catalog.sourceField(entry) : null;
    if (field) {
        const token = payload[field];
        const sourceUrl = typeof token === 'string' && token.includes('.') ? await sourceUrlFromToken(token, session) : null;
        if (!sourceUrl) {
            throw new GatewayError(400, 'source_required', 'Pick a clip you generated in this session to continue from.', { field });
        }
        return { payload: catalog.applyResolvedSource(entry, payload, { sourceUrl }), sourceUrl };
    }
    if (typeof payload.request_id !== 'string') {
        delete payload.request_id;
        return { payload, sourceUrl: null };
    }
    const token = payload.request_id;
    delete payload.request_id;
    if (!token.includes('.')) return { payload, sourceUrl: null };
...
function hasSteps(catalog, entry) {
    if (typeof catalog?.stepsOf !== 'function') return false;
    const { before, after } = catalog.stepsOf(entry);
...
export async function submitCatalogJob({ key, payload, session }) {
    ...
    const raw = { ...(payload || {}) };
    const sync = raw.sync === true;
    delete raw.sync;
    const { payload: body, sourceUrl } = await resolveReferencedJob(catalog, entry, raw, session);
    if (hasSteps(catalog, entry)) {
        // Multi-step entries (last frame → model → merge, or model → 4K
        // upscale) run as a server pipeline; the input is validated there
        // exactly like a direct submit.
        const { startPipelineJob } = await import('./pipelines/index.js');
        const { body: started } = await startPipelineJob({ name: 'catalog-steps', input: { key, payload: body, source_url: sourceUrl }, session, internal: true });
        return started;
    }
...
export async function runFal({ key, endpoint, input, session, signal, sync = false, estUsd, timeoutMs, onStatus, asStep = false }) {
    if (!isConfigured()) throw errors.notConfigured('The AI service');
    ...
    if (key) {
        ...
        // Extend / multi-step entries need a prior clip of the session and
        // extra server steps: only the catalog-steps pipeline runs them.
        if (!asStep && (entry.resolve || hasSteps(catalog, entry))) {
            throw new GatewayError(400, 'studio_only', 'This model only runs from the Video studio.');
        }
