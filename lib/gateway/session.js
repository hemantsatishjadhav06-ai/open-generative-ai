...
// Open gate: each visitor gets a private workspace ("v_" + random id), so
// strangers never see each other's agents, workflows or designs.
export const VISITOR_WORKSPACE_PREFIX = 'v_';

export function newVisitorWorkspace() {
    return `${VISITOR_WORKSPACE_PREFIX}${newId(12)}`;
}
...
// - open           → valid cookie, or a fresh visitor session (cookie set on reply)
...
export function requireSession(request) {
    ...
    if (mode === 'open') {
        const fresh = createSessionCookie({ codeId: newVisitorWorkspace(), secure });
